        ...
        <div className="absolute z-50 mt-2 w-[680px] md:w-[760px] bg-white rounded-2xl shadow-lg p-4 popover-animate origin-top-center">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="px-4 py-2 bg-expBlue text-white rounded-lg text-sm" onClick={apply}>Valider</button>
            </div>
          </div>
          ...
          <div className="flex gap-6">
            <div className="flex-1">
              <DatePicker
                selected={localStart}
                onChange={(dates: any) => { const [s, e] = dates as [Date | null, Date | null]; setLocalStart(s); setLocalEnd(e); }}
            ...
            <div className="w-64">
              ...
              <div className="flex flex-wrap gap-2">
                {[0,1,2,3,7].map((d) => (
                  <button key={d} onClick={() => setTolerance(d)} className={`px-3 py-2 rounded-full border text-sm ${tolerance===d? 'bg-expBlue text-white border-expBlue' : 'text-gray-600 bg-white'}`}>
